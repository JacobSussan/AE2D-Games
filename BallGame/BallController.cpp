...
{
	for (int i = 0; i < (int)grid->m_cells.size(); i++) {
		...
		for (int j = 0; j < (int)cell.balls.size(); j++) {
			...
			//check cells
			checkCollision(ball, cell.balls, j + 1);                                         ///< update own cell
			if (x > 0) {                                                                     ///
				checkCollision(ball, grid->getCell(x - 1, y)->balls, 0);                     ///< update left cell
				if (y > 0) {                                                                 ///
					checkCollision(ball, grid->getCell(x - 1, y - 1)->balls, 0);             ///< update top left cell
				}                                                                            ///
				if (y < grid->m_numYCells - 1) {                                             ///
					checkCollision(ball, grid->getCell(x - 1, y + 1)->balls, 0);             ///< update bottom left cell
				}                                                                            ///
			}                                                                                ///
			if (y > 0) {                                                                     ///
				checkCollision(ball, grid->getCell(x, y - 1)->balls, 0);                     ///< update top cell
			}
		}
	}
}
