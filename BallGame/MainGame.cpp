...
struct BallSpawn
{
	BallSpawn(const AE::Color_rgb8& colr,
...
{
	m_grid = std::make_unique<Grid>(m_screenWidth, m_screenHeight, CELL_SIZE); 	// init grid
...
#define ADD_BALL(p, ...) \
	...
	{
		ADD_BALL(1.0f, AE::Color_rgb8(r2(randomEngine), r2(randomEngine), r2(randomEngine), 255),
    ...
    for (int i = 0; i < NUM_BALLS; i++) {
        ...
        m_balls.emplace_back(ballToSpawn->radius, 
                        ...
                        AE::ResourceManager::getTexture("Textures/circle.png").id,
                        ballToSpawn->color);
 		//Add ball to grid. Don't call emplace back after this or you will break everything.
...
{
    const AE::Color_rgb8 fontColor(255, 0, 0, 255);
...
void MainGame::processInput() 
{
        ...
        switch (evnt.type) {
            ...
            case SDL_MOUSEMOTION:
                m_ballController.onMouseMove(m_balls, 
								(float)evnt.motion.x, 
								(float)m_screenHeight - (float)evnt.motion.y);
            ...
            case SDL_MOUSEBUTTONDOWN:
                m_ballController.onMouseDown(m_balls, 
								(float)evnt.button.x, 
								(float)m_screenHeight - (float)evnt.button.y);
    ...
    if (m_inputManager.isKeyJustPressed(SDLK_LEFT)) {
        m_ballController.setGravityDirection(GravityDirection::LEFT);
    } else if (m_inputManager.isKeyJustPressed(SDLK_RIGHT)) {
        m_ballController.setGravityDirection(GravityDirection::RIGHT);
    } else if (m_inputManager.isKeyJustPressed(SDLK_UP)) {
        m_ballController.setGravityDirection(GravityDirection::UP);
    } else if (m_inputManager.isKeyJustPressed(SDLK_DOWN)) {
        m_ballController.setGravityDirection(GravityDirection::DOWN);
    } else if (m_inputManager.isKeyJustPressed(SDLK_SPACE)) {
        m_ballController.setGravityDirection(GravityDirection::NONE);
    }

	if(m_inputManager.isKeyJustPressed(SDLK_1)){
		m_currentRenderer = 0;
		}
	if(m_inputManager.isKeyJustPressed(SDLK_2)){
		m_currentRenderer = 1;
	}
	if(m_inputManager.isKeyJustPressed(SDLK_3)){
		m_currentRenderer = 2;
	}
	if(m_inputManager.isKeyJustPressed(SDLK_4)){
		m_currentRenderer = 3;
	}
}
